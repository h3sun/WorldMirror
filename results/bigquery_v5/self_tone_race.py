"""
Bar Chart Race: 各国自我报道情感极性竞速（含国旗）
...
def sorted_ranking(pool):
    return sorted(pool.items(), key=lambda x: -x[1])
...
fig.text(0.065, 0.945, "自我报道最正面的国家排行榜",
         color="white", fontsize=30, fontweight="bold", va="center")

badge_country = fig.text(0.98, 0.02, "", color="#58a6ff",
                          fontsize=20, fontweight="bold", ha="right")
badge_count   = fig.text(0.98, 0.065, "", color="#8b949e",
                          fontsize=17, ha="right")
fig.text(0.02, 0.005,
         "指标：avg_tone（媒体报道自国的情感倾向，右正左负）  ·  含全部 48 国",
         color="#555555", fontsize=15)
...
def update(fi):
    ...
    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.tick_params(colors="white", labelsize=17)
    ...
    for country, d in spec.items():
        ...
        if val >= 0:
            ax.text(val + span * 0.02, y, f"{val:+.3f}",
                    va="center", ha="left", color="white", fontsize=17, alpha=alpha)
        else:
            ax.text(val - span * 0.02, y, f"{val:+.3f}",
                    va="center", ha="right", color="white", fontsize=17, alpha=alpha)
    ...
    ax.tick_params(axis="x", colors="#555555", labelsize=16)
...
out = ("/Users/haihaosun/Desktop/GitHub/WorldMirror/results/"
       "bigquery_v5/self_tone_race.mp4")
writer = FFMpegWriter(
